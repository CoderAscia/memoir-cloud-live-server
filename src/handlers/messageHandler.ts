...
export async function handleMessage(context: Context, parsedMessage: any) {
  ...
  if (parsedMessage.type === "getMessages") {
    ...
    socket.send(JSON.stringify({ type: "createConversationResponse", status: "Opps! It's not yet implemented.", data: {} }));
    return;
    ...
    const conv = await db.conversations.findOne({ conversationId }) as ConversationDocument | null;
    if (!conv) {
