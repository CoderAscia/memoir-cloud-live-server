...
ws.on('open', async () => {
    ...
    try {
        ...
        console.log("\n Testing existing conversation chat...");
        p = waitForResponse("aiChatResponse");
        ...
        console.log(`\n Testing delete messages from conversation id...${conversationId}`);
        ...
        console.log(`\n Testing delete conversation...${conversationId}`);
        const deleteConvo = await fetch(`${url}/conversations/${conversationId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'x-test-uid': userId
